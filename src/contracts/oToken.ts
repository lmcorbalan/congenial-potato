...
export class oToken extends SmartContract {
  static async build(web3: Web3, address: string): Promise<oToken> {
    ...
    const builder = oToken.configure({ contractName: 'oToken', abi, networks })
  ...
  async isUnsafe(vaultOwner: string) {
    const unsafe = await this.contract.methods.isUnsafe(vaultOwner).call()

    return unsafe
  }

  async maxOTokensLiquidatable(vaultOwner: string) {
